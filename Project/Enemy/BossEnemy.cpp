...
void BossEnemy::Load()
{
	...
	m_Handle = MV1LoadModel("Data/Enemy/NormalEnemy/BossEnemy.x");
	...
	if (m_AABB)
	{
		...
		m_AABB->SetLocalPos(VGet(0.0f, 8.0f, 0.0f));
...
// 索敵範囲内にプレイヤーがいるか判定 
bool BossEnemy::DetectPlayer(Player* player)
{
	// プレイヤーまでのベクトル 
...
EnemyBase* BossEnemy::Clone()
{
    ...
    // コピー代入演算子は使えないので、メンバーを個別にコピー
    // EnemyBaseのメンバー
    clone->SetName(m_Name);
    clone->SetMaxHP(m_MaxHP);
    clone->SetHP(m_HP);

    // BossEnemy独自のメンバー
    clone->m_State = m_State;
    clone->m_LostTimer = m_LostTimer;
    clone->m_CurrentMuzzle = m_CurrentMuzzle;
    clone->m_ShootTimer = m_ShootTimer;
    clone->m_DetectRange = m_DetectRange;
    clone->m_PatrolTimer = m_PatrolTimer;
    clone->m_PatrolDir = m_PatrolDir;
    for (int i = 0; i < MUZZLE_NUM; ++i)
    {
        clone->m_MuzzleBone[i] = m_MuzzleBone[i];
    }
    ...
    // AABB は必ず作り直す（ターゲットはクローンの m_Pos）
    ...
    if (clone->m_AABB)
    {
        ...
        clone->m_AABB->SetSize(VGet(1.0f, 2.0f, 1.0f));
    }
...
void BossEnemy::ChasePlayer(Player* player)
{
	...
	m_Pos = VAdd(m_Pos, VScale(dir, 0.05f));
...
// マズルのワールド座標を取得
void BossEnemy::GetMuzzleWorldPos(VECTOR& outPos)
{
